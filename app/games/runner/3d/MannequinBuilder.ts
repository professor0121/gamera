...
import { PlayerConfig, SkeletalJoints } from './Types.js';
import { MaterialFactory, GeometryFactory } from './Utility.js';

export function buildMannequin(mesh: THREE.Group, config: PlayerConfig): SkeletalJoints {
  ...
  const makeArm = (isLeft: boolean): { shoulder: THREE.Group; elbow: THREE.Group } => {
  ...
  const makeLeg = (isLeft: boolean): { hip: THREE.Group; knee: THREE.Group } => {
    const dir = isLeft ? -1 : 1;
