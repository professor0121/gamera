...
import { WeatherState } from './Types.js';
import { WEATHER_PRESETS } from './Constants.js';

export class MaterialManager {
  private static materials: Map<string, THREE.Material> = new Map();
  private static weatherMode: WeatherState = 'DAY';
  
  // Weather particles elements
  private static weatherParticles: THREE.Points | null = null;
  private static particleGeometry: THREE.BufferGeometry | null = null;
  private static particlePositions: Float32Array | null = null;
  private static particleCount = 600;

  static getMaterial(color: number, roughness = 0.5, metalness = 0.1, flatShading = true): THREE.Material {
  ...
  private static createParticles(mode: WeatherState, scene: THREE.Scene) {
  ...
  static updateWeather(delta: number, playerZ: number) {
